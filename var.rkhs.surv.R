var.rkhs.survival <- function(times, failures, variable, weights=NULL)
{
  if(sum(times<0)>0){print("Error times must be positive")
    ...
    if(sum(weights<=0)>0){print("Error weights must be superior to 0")
      ...
      if(sum(failures!=0 & failures!=1)>0){print("Error failures must be a vector of 0 or 1")
        ...
        table.surv <- data.frame(times=NULL, n.risk=NULL, n.event=NULL, survival=NULL, variance = NULL, variable=NULL)
        for(i in  unique(variable)){
          ...
          .st <- cumprod((.nj-.dj)/.nj)
          .mj <- sapply(.tj, function(x){(sum(.d$w[.d$t>=x]))^2/sum((.d$w[.d$t>=x])^2)})
          .vt <- (.st)^2*cumsum(.dj/(.mj * (.nj - .dj)))
